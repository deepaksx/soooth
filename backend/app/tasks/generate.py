...
async def run_generation_pipeline(job_id: str, db_session_factory):
    ...
    try:
        ...
        if job.video_source == "stock":
            ...
            clips_task = search_and_download_videos(search_term, target_duration=job.duration)
        else:
        ...
        if job.upload_youtube == "true":
            ...
            try:
                ...
                title = f"{theme_label} - Soothing {job.duration // 60}min Relaxation Video"
                description = (
                    f"A {job.duration // 60}-minute soothing {theme_label.lower()} video "
                    f"with AI-generated ambient music. Perfect for relaxation, meditation, "
                    f"study, sleep, and focus.\n\n"
                    f"Generated by Soooth - AI Soothing Video Generator"
                )
                tags = ["soothing", "relaxing", "nature", "ambient", "calm",
                        "meditation", "sleep", "study", "focus", theme_label.lower()]
                video_id = upload_to_youtube(str(output_path), title, description, tags)
